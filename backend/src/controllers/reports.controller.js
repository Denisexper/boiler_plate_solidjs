...
export const generateExcelReport = async (req, res) => {
    try {
        ...
        const logs = await Log.find(filter)
            ...
            .sort({ createdAt: -1 })
            .limit(1000); // Límite de 1000 registros
        ...
        if (startDate && endDate) {
            filterTexts.push(`Fechas: ${new Date(startDate).toLocaleDateString('es-ES')} - ${new Date(endDate).toLocaleDateString('es-ES')}`);
        }
        filterCell.value = filterTexts.length > 0 
        ...
        // Footer
        worksheet.addRow([]);
        const footerRow = worksheet.addRow([`Total de registros: ${logs.length}`, '', '', '', '', '', `Generado: ${new Date().toLocaleString('es-ES')}`]);
...
export const generatePDFReport = async (req, res) => {
    try {
        ...
        logs.forEach((log, index) => {
            ...
            doc.text(log.targetUser?.name || log.targetUserName || '-', 350, y, { width: 100 });
        ...
        doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
        doc.fontSize(10);
        doc.text(`Total de registros: ${logs.length}`, 50, doc.y + 10);
